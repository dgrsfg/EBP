...
<html>
  <head>
    ...
    <title>My JSP 'UpdateTicket.jsp' starting page</title>
    <style type="text/css">
  ...
  <body>
     <s:form action="/admin/UpdateTicketAction" theme="simple" method="post">
                 描述<textarea name="descs" cols="40" rows="4" id="textlimit1">${ticket.descs}</textarea>
                 &nbsp;还有<span class="counter1"></span>字符可以输入<br />
                  &nbsp;&nbsp;<span class="descs1"></span><br />        
                 时间:<input type="text"  id="datepicker" name="startTime" value="${ticket.startTime}"/><br />
                 &nbsp;&nbsp;<span class="startTime"></span><br /> 
   	  总票数:<input type="text" name="amount" value="${ticket.amount}" id="textlimit2"/>
   	             &nbsp; 还有<span class="counter2"></span>字符可以输入<br />
   	             &nbsp;&nbsp;<span class="amount"></span><br />  
  	  剩余票数:<input type="text" name="balance" value="${ticket.balance}" id="textlimit3"/>
  	             &nbsp; 还有<span class="counter3"></span>字符可以输入<br />
  	             &nbsp;&nbsp;<span class="balance"></span><br />  
               单价:<input type="text" name="price" value="${ticket.price}" id="textlimit4"/>
                &nbsp; 还有<span class="counter4"></span>字符可以输入<br /> 
                &nbsp;&nbsp;<span class="price"></span><br /> 
        <input type="hidden" name="tid" value="${ticket.tid}"/>
    <s:if test="#request.ticket.status == 0">           
              状态:<input type="radio" name="status" value="0" checked="checked"/>已停售
        <input type="radio" name="status" value="1"/>售票中                   
    </s:if>
    <s:elseif test="#request.ticket.status == 1">
